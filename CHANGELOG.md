# 更新日志 (Changelog)

本项目所有版本变更记录。GitHub Release 保留用户简明摘要，详细内容记录于本文件。

---

## [未发布]

### 优化

- 作品详情页升级双态沉浸顶栏架构：移除硬编码黑色渐变压暗，大图态展示液态玻璃浮动胶囊，向上滚动动态平滑过渡至 MIUIX 毛玻璃顶栏与作品信息。
- 全面优化主导航栏各页（排行榜、关注动态、特辑、搜索）顶栏结构与全屏采样穿透，将分类选择器与搜索控件一体化整合入毛玻璃顶栏，彻底解决上滑列表时毛玻璃模糊失效的问题。
- 排行榜日期筛选升级为顶栏紧凑日历弹窗交互，关注动态与特辑分类横条升级为原生 MIUIX 质感胶囊分段条。
- 重构全工程顶栏为官方规范的渐进式纹理毛玻璃架构（`BlurredBar` + `ProgressiveBlur.Top`），实现非侵入式包装与自适应动态高度，消除 56dp 硬编码截断与展开镂空。
- 升级 `LiquidGlass` 物理液态玻璃与模糊底栏降级体系，支持跨平台与无运行时着色器环境下优雅回退。
- 全量打通所有一级页面、二级页面与设置子页面的 MIUIX Backdrop 顶部毛玻璃模糊管线，与底栏保持统一通透磨砂质感。
- 作品详情页与用户主页简介全面升级超链接解析引擎，支持自动识别纯文本 URL、Pixiv 快捷 ID、出站跳转（jump.php）智能解包直达及站内全链路深层路由分发。
- 优化画师个人主页为原生 MIUIX 大标题折叠联动架构，头部与 Tab 栏随瀑布流列表平滑滚动折叠，大幅扩充作品展示面积。
- 优化作品详情页顶部操作栏，校准下载按钮图标视觉居中，并将更多操作菜单全面升级为同材质的液态玻璃浮动弹窗与微交互动效。
- 深度集成 Liquid Glass 液态玻璃物理特效体系至全局灵动胶囊 Toast、作品详情页悬浮控制工具栏、排行榜分段滑块与瀑布流微型玻璃徽标。
- 全面升级登录页与初次启动引导流程，重塑 MIUIX 风格品牌 Hero 视觉、动态伸缩胶囊步骤指示器及大屏自适应排版。
- 登录页接入剪贴板凭证智能读取、受限网络环境排障引导卡片与服务条款政策外链。
- 优化多端与全尺寸屏幕开箱体验，首次启动默认按 180dp 卡片基准宽度自适应列数（手机 2 列、平板 3~4 列、桌面 5~8 列）。
- 升级 MIUIX 基础架构至 0.9.4-rc01 并同步升级 Compose Multiplatform 至 1.12.0-rc01，引入 BreadcrumbBar 面包屑路径导航。
- 全面接入 miuix-preference 架构，统一设置页与偏好控件为 SwitchPreference、OverlayDropdownPreference、RadioButtonPreference、ArrowPreference 与 SliderPreference 原生组件。
- 接入 ColorPicker 拾色器、NumberPicker 滚轮选择器、OverlayIconDropdownMenu 锚点菜单、Badge 角标与 TooltipBox 提示气泡等 MIUIX 原生交互组件。
- 实现自适应多列瀑布流适配器，为长列表与插画交错瀑布流（首页推荐、排行榜、搜索结果、历史记录）全量集成 VerticalScrollBar 垂直滚动条。
- 全面升级并统一毛玻璃特效架构，多平台（Android / Desktop / iOS / macOS）深度集成 Backdrop 实时模糊与液态玻璃折射体系。
- 全面重构悬浮底栏为生产级三态液态玻璃体系，深度集成物理色散透镜折射、陀螺仪重力感应高光、触摸光晕聚焦与双重背景采样。
- 底部导航栏双模式（标准底栏与悬浮底栏）接入原生动态未读徽标与按压触感反馈。
- 补充与完善全平台项目文档、使用指南及应用内关于页免责声明。
- 启用 R8 Full Mode 代码混淆与资源精简，大幅缩减 APK 安装包体积。
- 规范化多平台沙盒隔离的 Coil 3 本地磁盘缓存与后台网络连接预热，大幅缩减冷启动与首屏等待耗时。
- 接入当前激活底栏再次点击平滑回顶并刷新交互，配合触感反馈。
- 接入 MIUIX 触感反馈系统，覆盖卡片长按快捷操作、收藏点赞与回顶操作。
- 补齐 Android 13+ 通知权限声明与常规 Pixiv 网页链接及系统分享外部拦截。
- 接入前台智能识别剪贴板作品链接与主页双击返回退出防误触。
- 桌面端新增最小窗口尺寸约束、全局 ESC 快捷键返回与网络预热。
- 完善 Android 16 实时动态胶囊通知与 Xiaomi HyperOS 灵动状态栏焦点通知协议。
- 注入规范化 Baseline Profiles 基准规则，强化 ART 虚拟机 AOT 预编译与冷启动表现。
- 液态玻璃悬浮底栏集成阻尼拖拽弹性物理动效与磁吸入位波形联动。
- 升级跨应用全局拖拽与系统级传送门提取引擎，支持直接无损提取全分辨率原图及多页索引精准匹配。
- 优化初次启动向导界面，引导流程聚焦语言选择并移除冗余贡献者板块，贡献者仅在设置中心保留展示。


### 修复

- 修复顶栏在折叠展开过程中出现的物理断层、光学硬截断以及人工补线痕迹，平滑衔接列表瀑布流内容。
- 评论页顶栏回归官方标准纯色背景，彻底根除滑动时文本被模糊成色块的问题。
- 调优下拉刷新指示器垂直基准坐标，精确对齐展开顶栏底沿与列表卡片衔接处。
- 修复顶栏大标题未折叠状态下模糊采样溢出导致的分类标题块状光晕与分界线切割问题，并将滚动条隔离于 Backdrop 图层外。
- 修复顶栏与底栏透明度不一致、滑动时背景抖动及设置页分类标题被单独模糊产生明显分界线的问题。
- 修复顶栏毛玻璃模糊在页面切换与滑动时可能出现的偶发纯灰底色遮罩问题，优化 Backdrop 页面隔离与 Vibrancy 渲染通道。
- 修复悬浮底栏指示器手势拦截导致再次点击当前激活标签无法触发平滑回顶与刷新的问题。
- 修复悬浮底栏选择指示器在外部界面与标签切换时未响应同步移动的问题。
- 修复悬浮底栏在多次快速切换时失灵及单向数据流锁死问题。
- 重构一级主标签容器为 HorizontalPager 架构，彻底消除反向切页时的右侧灰白残影与白边，实现全页面物理连续滑动与状态保持。
- 修复语言设置等二级页面与搜索建议热门标签因内边距与嵌套滚动冲突导致的内容消失问题。
- 修复跨应用全局拖拽在系统级中转站中识别为链接而非图片并发生权限闪退的问题。
- 修复关注动态页右上角未读红点无法被点击或阅读消除的问题。
- 修复顶栏 Backdrop 采样自嵌套导致的 Android 底层 HWUI 渲染递归与崩溃问题。
- 修复页面栈路由推进重复配置及恢复前台时剪贴板深度跳转导致的异常。
- 修复主题设置修改后未即时全局响应以及 AMOLED 纯黑模式下的前景色对比度问题。

---

## [v0.9.108.2-miuix] - 2026-08-24

### 新增

- 支持桌面小部件无缓存时自动在线拉取与更新。
- 支持独立设置桌面小部件的内容推荐分类与图片代理源。
- 支持作品详情页在高清大图加载完成前优先展示已缓存缩略图。
- 接入预测性返回与视差平移手势，统一左至右返回出栈。
- 支持应用内直接流式下载更新安装包并自动唤起系统安装器。
- 支持版本更新弹窗与更新设置页面富文本 Markdown 日志渲染。
- 接入 Android 16 实时动态胶囊下载通知与现代化照片选择器。
- 接入 Android 14+ 桌面快捷捷径、深度链接路由与系统级分享动作。
- 接入 Android 14+ 广色域渲染与动态取色自适应图标。
- 评论区支持 Pixiv 官方表情包解析与快捷输入面板。
- 接入系统存储目录选择器，支持通过系统文件框架直接选择保存目录。
- 插画详情页支持开启左右手势滑动切换作品。

### 修复

- 修复桌面小部件初始化黑屏无内容的问题。
- 修复作品详情页与用户详情页右上角操作菜单点击无响应的问题。
- 修复画质档位设置文案映射错位的问题。
- 修复列表流中 R-18 内容过滤设置未生效的问题。
- 修复横屏模式下列数未正确响应配置的问题。
- 修复插画详情页未自动记录本地浏览历史的问题。
- 修复返回手势中途消失与页面切换闪烁的问题。

### 优化

- 全面重构设置中心卡片层级，规范归类画质、保存、浏览、外观与隐私等配置。
- 补齐收藏与下载自动化联动偏好，新增收藏下载状态反馈。
- 优化长列表滑动与网格流渲染性能，降低高速滑动重组耗时。
- 移除背景帧率硬编码限制，原生适配高刷屏幕。
- 全面完善 11 种语言本地化翻译支持。
- 增强跨平台登录凭证与授权回调链接智能解析。

---

## [v0.9.108.1] - 2026-08-18

### 新增

- 登录界面新增直接输入 Access Token 快速登录功能，方便跨平台授权与登录凭证同步。
- 全面完善 11 种语言本地化支持（简体中文、繁体中文、英语、日语、韩语、俄语、西班牙语、土耳其语、印尼语、菲律宾语、德语），补齐所有设置界面的多语言翻译。

### 修复

- 修复 Android 端页面返回异常退出至桌面以及 HyperOS 预测性返回手势动画未生效的问题。
- 修复 Android 端在横竖屏旋转切换时触发 Activity 重建并导致页面重新刷新的问题。
- 修复平板端与宽屏设备无法正确切换悬浮底栏的问题，支持在左侧 MIUIX NavigationRail 侧边栏与居中悬浮底栏间无缝切换。
- 修复桌面端与平板端宽屏模式下残留 Material 3 控件的问题，全面迁移至 MIUIX 官方 NavigationRail 侧边栏与超椭圆组件规范。
- 修复桌面端完成登录后未能自动跳转回主界面的问题。

---

## [v0.9.108] - 2026-08-18

### 新增

- 首页推荐 (Hello)、关注动态 (New)、插画与画师搜索 (Search)、排行榜 (Ranking)、相关推荐 (Related)、插画/漫画系列详情 (Series)、画师主页作品与收藏 (UserDetail)、作品评论区 (Comments) 以及好P友/粉丝列表全量支持触底自动流式预加载。
- 全部列表页面接入 MIUIX 官方下拉刷新与顶部手动刷新按钮。
- 新增项目架构全景百科文档（`Code_Wiki.md`）、UI 设计与组件规范（`MIUIX_Spec.md`）及全局待办清单（`Global_TODO.md`）。

### 优化

- 悬浮胶囊底栏增加最大宽度限制（320dp ~ 540dp）并在平板与桌面大屏设备上居中对齐，消除全屏拉伸失真。
- 瀑布流在大屏和宽屏设备上自适应 3 到 7 列布局，单列内容容器居中限宽（760dp）。
- 升级推荐用户页为触底自动平滑加载。

### 修复

- 修复长评作品评论区无法加载历史评论的问题。
- 修复画师关注列表和好P友/粉丝列表仅加载首批数据的问题。
- 修复超过 30 话的连载插画/漫画系列详情页无法加载完整章节的问题。
- 修复 Android 端 Release 发行包构建校验异常。



---

## [v0.9.105] - 2026-08-17

### 新增

- PixEz MIUIX (Compose Multiplatform + Xiaomi HyperOS / MIUIX) 重构首发版发布。
- 支持 IosLiquidGlass 悬浮液态玻璃底栏（高斯模糊基底、动态高光拾取、色散折射透镜层）与阻尼手势拖拽。
- 支持 Android 13+ 原生 `RuntimeShader` 与 Desktop Skia `RuntimeEffect` 运行时 GLSL 动态流光背景。
- 全量采用 Xiaomi HyperOS (MIUIX) 官方组件体系（Squircle 超椭圆圆角卡片、`SuperDialog` 弹窗、色彩系统）。
- 支持免代理 SNI 混淆直连引擎与 OAuth2 PKCE 自动鉴权与 Token 无感刷新。
- 支持插画/漫画多图轮播浏览与动图 (Ugoira) 原生帧播放。
- 支持 Pixivision 官方特辑沉浸式图文阅读器。
- 支持作品 ID、画师 ID、标签及 AI 作品过滤等多重屏蔽引擎。
- 支持多任务并发下载队列与本地 MediaStore 相册同步。
