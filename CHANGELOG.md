...
- 修复 Android 桌面小部件初始化黑屏问题，新增 `WidgetRepository` 支持无本地缓存时按需自动网络拉取与持久化；支持独立设置小组件内容推荐类型（日榜/周榜/月榜/推荐/最新/关注等）与独立图片代理 CDN 源。
- 优化作品详情页大图加载体验，支持原图/高清图加载完成前优先展示已缓存的缩略图占位（`thumbnailUrl`），消除加载空白与闪烁。
- 重构 MIUIX / HyperOS 预测性返回与页面平移体系（`miuixSlidePredictiveBackAnimatable` / `miuixSlideStackAnimation`），支持标签智能双向平移、100% 视口全行程位移、底层 30% 视差滑入与柔和遮罩，统一左至右返回出栈，彻底消除闪烁与中途消失。
- 补齐「收藏后保存」的下载状态反馈 Toast，并在应用启动及设置页中接入全局版本更新弹窗（`UpdateDialog`）与“NEW”高亮徽标提示。
- 全面审查并修正设置页面全部标题、副标题与选项文案，解决“屏幕刷新率”显示为“显示模式”、自适应列数文案错配等问题，重构交互与画质卡片分类。
