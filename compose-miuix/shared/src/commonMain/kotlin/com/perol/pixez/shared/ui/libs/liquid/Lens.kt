 ...
 *
 * @param refractionHeight Distance from border over which refraction is applied.
 * @param refractionAmount Magnitude of refraction coordinate distortion.
 * @param depthEffect Whether to add spherical depth distortion.
 * @param chromaticAberration Strength of the rim chromatic dispersion. `0f` disables the
 *  effect (cheaper non-dispersion shader is used). Typical values: `0.1` for subtle,
 *  `0.2` for Apple-pill-like, `0.3+` for pronounced rainbow halo.
 */
...
/**
 * Boolean-overload for lens chromatic aberration.
 */
fun BackdropEffectScope.lens(
    refractionHeight: Float,
    refractionAmount: Float,
    ...
    lens(
        ...
        chromaticAberration = if (chromaticAberration) 0.15f else 0f,
    )
}
...
half4 main(float2 coord) {
    ...
    half4 orange = content.eval(refractedCoord + dispersedCoord * 0.6667);
    half4 yellow = content.eval(refractedCoord + dispersedCoord * 0.3333);
    half4 green = content.eval(refractedCoord);
    half4 cyan = content.eval(refractedCoord - dispersedCoord * 0.3333);
    half4 blue = content.eval(refractedCoord - dispersedCoord * 0.6667);
    half4 purple = content.eval(refractedCoord - dispersedCoord);

    color.r = red.r * 0.30 + orange.r * 0.25 + yellow.r * 0.25 + purple.r * 0.20;
    color.g = orange.g * 0.15 + yellow.g * 0.25 + green.g * 0.30 + cyan.g * 0.30;
    color.b = cyan.b * 0.30 + blue.b * 0.40 + purple.b * 0.30;
