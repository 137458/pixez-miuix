...
3. **时间范围筛选 UI**
   ...
   - 使用 MIUIX `InputField` 接收 `YYYY-MM-DD` 格式输入。
   - 提供「清空」按钮移除已选时间范围。
   - 输入有效日期后自动重新加载结果。

## 验收条件

- [ ] `SearchRepository.searchIllust` 支持 `startDate` / `endDate`。
- [ ] `SettingsKeys` 新增 `SEARCH_START_DATE`、`SEARCH_END_DATE`。
- [ ] `SearchScreen` 进入时恢复时间范围，变化时回写设置。
- [ ] 输入有效时间范围后，搜索结果按日期区间过滤。
- [ ] Android + Desktop 双端编译通过。
- [ ] M18 code review 完成，无 P0/P1 问题遗留。
