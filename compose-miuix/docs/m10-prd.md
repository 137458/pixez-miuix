...
- [x] 搜索结果页顶部显示排序与目标筛选 TabRow。
- [x] 切换排序后重新加载对应排序结果。
- [x] 切换搜索目标后重新加载对应目标结果。
- [x] Android + Desktop 双端编译通过。
- [x] M10 code review 完成，无 P0/P1 问题遗留。

## Code Review 记录

本次 review 未发现 P0/P1 问题。P2 优化项（非必须）：`SearchFilterBar` 中的选项列表可在重组时提取为 `remember` 或顶层常量，当前因任务最小化原则暂不处理。
